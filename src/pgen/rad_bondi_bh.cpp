...
namespace {
    ...
    struct pgen_bh{
        ...
        r_vir, rho_vir, M_bh, rho_cgm, cs_cgm, epsilon, gamma_gas, v_bh, 
...
namespace{
    ...
    void AddBHGrav(Mesh *pm, const Real bdt, DvceArray5D<Real> &u0,
        ...
        KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
            ...
            int nx3 = indcs.nx3;
            Real x3v = CellCenterX(k-ks, nx3, x3min, x3max);
            Real x1new = x1v-pbh->v_bh*time;
            Real rad = sqrt(SQR(x1new)+SQR(x2v)+SQR(x3v));

            
            Real grad_phi_by_r = (pbh->CONST_G*pbh->M_bh)/(pow((SQR(rad)+SQR(pbh->epsilon)),1.5));

            Real pm1 = u0(m, IM1, k, j, i);
            Real pm2 = u0(m, IM2, k, j, i);
            Real pm3 = u0(m, IM3, k, j, i);
            Real rho = u0(m, IDN, k, j, i);

            u0(m,IM1,k,j,i) -= (rho*grad_phi_by_r*bdt)*x1new;
            u0(m,IM2,k,j,i) -= (rho*grad_phi_by_r*bdt)*x2v;
            u0(m,IM3,k,j,i) -= (rho*grad_phi_by_r*bdt)*x3v;
            Real p_dot_r = pm1*x1new+pm2*x2v+pm2*x3v;
