...
namespace {
  ...
  struct pgen_jet_amb{
      ...
      Real cs_amb, d_amb, rho_jet, gamma, r_jet, l_jet, v_jet, alpha_jet, theta_jet;
  };
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  pjet->cs_amb        = pin->GetOrAddReal("problem", "cs_amb",1);
  ...
  pjet->alpha_jet     = (M_PI/180.)*pin->GetReal("problem", "alpha_jet_d");
  pjet->theta_jet     = (M_PI/180.)*pin->GetReal("problem", "theta_jet_d");


  Real area_jet       = M_PI*pjet->r_jet*pjet->r_jet*sin(pjet->alpha_jet)*sin(pjet->alpha_jet);
  ...
  if (pmbp->phydro!=nullptr){
    ...
    KOKKOS_LAMBDA(int m,int k,int j,int i) {
      ...
      if (InJet(x1v, x2v, x3v, pjet->r_jet, pjet->alpha_jet, pjet->theta_jet)) {
        scal =1.0;
        u0(m,IDN,k,j,i) = pjet->rho_jet;
        Real        rad = sqrt(SQR(x1v) + SQR(x2v) + SQR(x3v));
        u0(m,IM1,k,j,i) = (rad > 0) ? pjet->rho_jet*pjet->v_jet*x1v/rad : 0.0;
        u0(m,IM2,k,j,i) = (rad > 0) ? pjet->rho_jet*pjet->v_jet*x2v/rad : 0.0;
        u0(m,IM3,k,j,i) = (rad > 0) ? pjet->rho_jet*pjet->v_jet*x3v/rad : 0.0;
      } else {
