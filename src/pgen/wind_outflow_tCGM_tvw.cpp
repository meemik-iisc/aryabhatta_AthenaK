...
namespace {
    ...
    struct PgenBh{
        ...
        M_bh, v_bh, epsilon,
        vw_0,alpha_vw, r_inj, r0,
        rho_cgm0, cs_cgm, alpha_cgm, gamma_gas, 
    ...
    static Real Rho_CGM(const Real t,const Real alpha_cgm, const Real rho_cgm0) {
        Real rho_cgm = rho_cgm0*std::pow((1+t),alpha_cgm);
        return rho_cgm;
    }
    KOKKOS_INLINE_FUNCTION
        static Real Vw(const Real t,const Real alpha_vw, const Real vw_0) {
            Real v_w = vw_0*std::pow((1+t),alpha_vw);
            return v_w;
    }
    KOKKOS_INLINE_FUNCTION
        static Real Mdot_w(const Real rho_cgm,const Real vw,const Real v_BH, const Real r0) {
            Real Mdot_w = (4*M_PI*SQR(r0)*rho_cgm*SQR(v_BH))/vw;
            return Mdot_w;
    }
} // namespace
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
    ...
    pbh.rho_cgm0       = pin->GetReal("problem","rho_cgm0");    
    ...
    pbh.vw_0           = pin->GetReal("problem","vw_0");
    pbh.alpha_vw       = pin->GetReal("problem","alpha_vw");
    pbh.r_inj          = pin->GetReal("problem","r_inj");
    pbh.r0             = pin->GetReal("problem","r0");
    ...
    if (pmbp->phydro!=nullptr){
        ...
        KOKKOS_LAMBDA(int m,int k,int j,int i) {
            ...
            if (rad<pbh.r_inj){
                Real vw         = Vw(time, pbh.alpha_vw, pbh.vw_0);
                Real rho_cgm    = Rho_CGM(time, pbh.alpha_cgm, pbh.rho_cgm0);
                Real mdot       = Mdot_w(rho_cgm, vw, pbh.v_bh, pbh.r0);
                Real v_inj      = vw/std::sqrt(2);
                Real rho_w      = (mdot)/(4*M_PI*SQR(rad)*v_inj);
    ...
    void AddISMCooling(Mesh *pm, const Real bdt, DvceArray5D<Real> &u0,
        ...
        KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
            ...
            if (temp_cgs<pbh.temp_floor){
                ...
                w0(m,IEN,k,j,i)         = pres_floor_code/gm1;
