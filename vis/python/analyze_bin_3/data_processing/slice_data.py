...
def extract_athenak_slice(user_params):
    ...
    Returns:
        ...
        "blocks": Block data
        "num_blocks": Number of blocks used,
        "block_shape": (block_nz, block_ny, block_nx)
    """
...
def extract_radial_vel_slice(user_params):
    ...
    velr_data = -1000+(np.sqrt(np.square(velx_data)+np.square(vely_data)+np.square(velz_data))*Velr_scale)
    return {
