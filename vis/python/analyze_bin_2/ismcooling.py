...
def cool_lambda(T):
    """
    SPEX cooling curve from Table 2 of Schure et al, A&A 508, 751 (2009).
    Covers 4.12 < logT < 8.16 K, with extensions for low/high temperatures.
    """
    # Tabulated log cooling rates (lhd) from Schure et al.
    lhd = np.array([
    ...
    # For T < 10^4.2 K, use Koyama & Inutsuka (2002)
    mask_low = logt <= 4.2
    result = np.zeros_like(T)
    result[mask_low] = (2.0e-19 * np.exp(-1.184e5 / (T[mask_low] + 1.0e3)) + 
                       2.8e-28 * np.sqrt(T[mask_low]) * np.exp(-92.0 / T[mask_low]))
    
    # For T > 10^8.15 K, use CGOLS fit
    mask_high = logt > 8.15
    result[mask_high] = 10.0 ** (0.45 * logt[mask_high] - 26.065)
    
    # For 4.2 < logT < 8.15, linear interpolation of SPEX table
    mask_mid = (logt > 4.2) & (logt <= 8.15)
    if np.any(mask_mid):
        logt_mid = logt[mask_mid]
        ipps = np.floor(25.0 * logt_mid - 103).astype(int)
