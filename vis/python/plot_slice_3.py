...
def main(**kwargs):
    ...
    pres_scale = 1.0
    tracer_scale = 1.0
    # tracer_scale = 1.0/20.0
    s_Myr       = 3.154e13 #seconds to Myrs    
    s_yr        = 3.154e7 #seconds to yrs    
    ...
    if kwargs['variable'][:8] != 'derived:':
        if kwargs['variable']=='dens':
            ...
            # print(f"*** s_00 RAW: min={quantity.min():.4f} max={quantity.max():.4f} frac1={(quantity==1.0).mean():.3f} ***")
        else:
    ...
    if kwargs['norm'] == 'linear':
        ...
        vmin = None
        vmax = None
    elif kwargs['norm'] == 'twoSlope':
        norm = TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)
        # Build negative ticks from -0.1 down to -1.0
        neg_ticks = np.arange(0.0, vmin-0.2, -0.2)
        # And positive ticks (adjust spacing to suit)
        pos_ticks = np.linspace(0, vmax, 6)
        all_ticks = np.concatenate([neg_ticks, pos_ticks])
        vmin = None
        vmax = None
    elif kwargs['norm'] == 'twoLog':
        norm = SymLogNorm(linthresh=1, linscale=1.0, vmin=vmin, vmax=vmax, base=10)
        vmin = None
        vmax = None
    else:
        norm = kwargs['norm']
    ...
    scaled_extents = [tuple(axes_scale * x for x in extent) for extent in extents]

    # Plot data
    # for block_num in range(num_blocks_used):
    #     im=plt.imshow(quantity[block_num], cmap=kwargs['cmap'], norm=norm, vmin=vmin,
    ...
    if kwargs['norm'] == 'twoSlope':
        ...
        cb.set_ticklabels([f"{t:.1f}" for t in all_ticks])
    ...
    if kwargs['r_max'] is not None:
        ...
        x2_max = kwargs['r_max']
    #Scale the axes
    x1_min, x1_max, x2_min, x2_max = [val * axes_scale for val in (x1_min, x1_max, x2_min, x2_max)]
